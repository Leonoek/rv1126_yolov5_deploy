...
/*
Source file     rkmedia_venc_jpeg_test.c
Modified by     Yuanming Li
Change time     2025/3/4
*/
...
void video_packet_cb(MEDIA_BUFFER mb) {
  ...
  char jpeg_path[128];
  sprintf(jpeg_path, "%s%d.jpeg", g_pOutPath, jpeg_id);
...
static RK_CHAR optstr[] = "?::a::w:h:W:H:o:I:M:r:q:";
...
static void print_usage(const RK_CHAR *name) {
  ...
  printf("\t-W | --Width: source width, Default:1280\n");
  printf("\t-H | --Height: source height, Default:1280\n");
  printf("\t-w | --width: destination width, Default:640\n");
  printf("\t-h | --height: destination height, Default:640\n");
  printf("\t-o | --output: output dirpath, Default:/tmp/\n");
  printf(
      "\t-r | --rotation: rotation control, Default:0. Values:0/90/180/270\n");
  printf("\t-q | --qfactor: quality of image, Default:99\n");
  printf("\t tip: destination resolution should not over 4096*4096\n");
}

int main(int argc, char *argv[]) {
  RK_S32 ret;
  RK_U32 u32SrcWidth = 1280;
  RK_U32 u32SrcHeight = 1280;
  RK_U32 u32DstWidth = 640;
  RK_U32 u32DstHeight = 368;
  ...
  while ((c = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
    ...
    case 'h':
      ...
      // if ((u32DstHeight % 16) != 0)
      // {
      //   printf("ERROR: u32DstHeight should be %% 16 == 0, example: 640, 352, 368 can %% 16 == 0\n");
      //   return(EXIT_FAILURE);
      // }
      break;
    case 'W':
  ...
  while (!quit) { 
    ...
    printf("#s32Y: %d, s32Y: %d, u32Width: %d, u32Height:%d\n", 0, 0, u32DstWidth, u32DstHeight);
    venc_chn_param.stCropCfg.stRect.s32X = 0;
    venc_chn_param.stCropCfg.stRect.s32Y = 0;
    venc_chn_param.stCropCfg.stRect.u32Width = u32DstWidth;
    venc_chn_param.stCropCfg.stRect.u32Height = u32DstHeight;
